...
class CreateStarshipImplTest {

	@Mock
	private StarshipRepository repository;

	@InjectMocks
	private CreateStarshipImpl service;

	@Nested
	class CreateSarshipTest {

		@Test
		void givenStarship_returnCreatedStarship() {
			//Given
			StarshipDto mockStarshipDto = new StarshipDto(1L, "New Starship", "New Description");
			Starship mockStarship = new Starship(1L, "New Starship", "New Description");
			Mockito.when(repository.save(Mockito.any(Starship.class))).thenReturn(mockStarship);

			//When
			StarshipDto result = service.createSarship(mockStarshipDto);
